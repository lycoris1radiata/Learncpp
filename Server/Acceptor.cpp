...
void Acceptor::NewConnection() {
    ...
    if(SetSockoptKeepAlive(connfd)==-1){
        //printf("acceptor:SetSockoptKeepAlive failed\n");
        LOG_ERROR("%s\n","Acceptor::NewConnection SetSockoptKeepAlive failed");
        close(connfd);
        return;
    }
    if(SetSockoptTcpNoDelay(connfd)==-1){
        //printf("acceptor:SetSockoptTcpNoDelay failed\n");
        LOG_ERROR("%s\n","Acceptor::NewConnection SetSockoptTcpNoDelay failed");
        close(connfd);
        return;
    }
