...
tiny_muduo::TimerQueue::TimerQueue(EventLoop *loop)
        :loop_(loop),
...
tiny_muduo::TimerQueue::~TimerQueue()
{
    channel_->DisableAll();
...
void tiny_muduo::TimerQueue::ResetTimer(Timer *timer)
{
...
void tiny_muduo::TimerQueue::AddTimer(Timestamp timestamp, BasicFunc &&cb, double interval)
{
